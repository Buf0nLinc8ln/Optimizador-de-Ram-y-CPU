...
GtkWidget *entry_pid, *entry_uid, *entry_gid, *entry_nombre, *entry_ppid, *entry_gpid, *entry_estado, *entry_ram;
...
static void aplicar_filtros(GtkButton *button, gpointer user_data) {
    ...
    char *ppid = g_strstrip(g_strdup(gtk_editable_get_text(GTK_EDITABLE(entry_ppid))));
    char *gpid = g_strstrip(g_strdup(gtk_editable_get_text(GTK_EDITABLE(entry_gpid))));

    int pid = atoi(pid_text);
    int uid = atoi(uid_text);
    int gid = atoi(gid_text);
    int ppid_num = atoi(ppid);
    int gpid_num = atoi(gpid);
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    gtk_label_new(
        ...
        "T (Suspendido), D (Espera ininterrumpida)"
    ),
    0, row++, 2, 1
    );
